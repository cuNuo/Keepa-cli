...
class CapabilitiesTests(unittest.TestCase):
    def test_service_capabilities_include_schema_version_and_graphimage(self):
        ...
        self.assertEqual(payload["data"]["schema_version"], "2026-05-11.12")
        ...
        self.assertIn("resource_uri", payload["data"]["mcp"]["workflow_runtime"]["argument_names"])
        self.assertEqual(payload["data"]["mcp"]["workflow_runtime"]["schema_resource_uri"], "keepa://schema/workflow-runtime-contract")
        self.assertGreaterEqual(payload["data"]["mcp"]["workflow_runtime"]["tool_count"], 8)
        ...
        self.assertIn("products_get", mcp_tool_names)
        self.assertIn("products_compare", mcp_tool_names)
        self.assertIn("categories_finder_selection", mcp_tool_names)
        self.assertIn("research_graph_merge", mcp_tool_names)
        self.assertIn("research_brief_export", mcp_tool_names)
        self.assertIn("docs_index", mcp_tool_names)
        self.assertIn("docs_read", mcp_tool_names)
        self.assertIn("context_policy", mcp_tool_names)
        self.assertIn("resolve_research_target", mcp_tool_names)
        self.assertIn("query_research_context", mcp_tool_names)
        self.assertIn("audit_cost", mcp_tool_names)
        self.assertIn("cassettes_promote", mcp_tool_names)
        self.assertIn("cassettes_promote_and_verify", mcp_tool_names)
        self.assertIn("reports_build", mcp_tool_names)
        self.assertIn("tracking_list", mcp_tool_names)
    ...
    def test_stdio_capabilities_returns_response_event(self):
        ...
        self.assertEqual(response["payload"]["data"]["schema_version"], "2026-05-11.12")
