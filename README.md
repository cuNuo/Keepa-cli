...
The isolated SDK adapter lives in `keepa_cli/agent/mcp_sdk_adapter.py`; run `python scripts/compare_mcp_sdk_adapter_fixture.py` to compare its compatibility handler with the current `--mcp` stdio output, run `python scripts/smoke_mcp_sdk_adapter_client.py` to connect with the official Python MCP SDK `ClientSession`, and run `python scripts/check_mcp_sdk_adapter_typed_fixture.py` to map the Inspector fixture into typed SDK calls. The optional `mcp-sdk` extra enables SDK experiments and the `python -m keepa_cli.agent.mcp_sdk_adapter --stdio` entrypoint, but it does not replace the production stdio entrypoint. The SDK adapter keeps `toolset=all` discoverable through cursor pagination, but its first page is capped to starter tools headed by `keepa.context_policy`.
