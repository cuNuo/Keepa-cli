...
The isolated SDK adapter spike lives in `keepa_cli/agent/mcp_sdk_adapter.py`; run `python scripts/compare_mcp_sdk_adapter_fixture.py` to compare its output with the current `--mcp` stdio handler. The optional `mcp-sdk` extra only enables SDK experiments and does not replace the production stdio entrypoint.
