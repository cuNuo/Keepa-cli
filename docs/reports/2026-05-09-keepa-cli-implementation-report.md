...
CLI 的核心价值不是把 REST API 简单包一层，而是把 Keepa 的 token 成本、分页/批量、gzip/keep-alive、`csv` 历史数据、错误响应与缓存策略固化成可审计的 Agent 工具层。默认模式可以像主流 Agent CLI 一样进入交互式工作台，支持搜索、选择、预览、确认、导出和历史记录；但所有底层能力必须先能被 Agent 通过 `--json`、`--stdio` 和显式子命令稳定调用。
...
- 主流 Agent CLI reference：内部归纳为非交互 JSON、交互工作台、slash/workflow 入口三类能力基线。
- 主流 Agent CLI interactive mode：内部归纳为命令历史、快捷动作、确认流和可回放输出。
- 主流 Agent CLI slash commands / skills：内部归纳为可发现命令、可组合工作流和本地 skill 入口。
...
主流 Agent CLI 的核心启发是默认进入交互会话，而不是要求用户记住大量参数。典型能力包括非交互输出、结构化 JSON/stream JSON、面向程序调用的输入格式、快捷键、命令历史、slash commands、shell 前缀和可切换模式。
