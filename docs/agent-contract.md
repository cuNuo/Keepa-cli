...
MCP stdio 当前返回 `protocolVersion=2025-11-25`，`serverInfo.name=keepa_mcp`；客户端配置里的服务器别名仍可继续使用 `keepa`。`tools/list` / `resources/list` / `resources/templates/list` / `prompts/list` 支持 MCP cursor 分页：请求可传 `cursor`，Keepa-cli 额外支持 `limit` 便于本地调试；响应在 `_meta.total_count`、`_meta.has_more` 与 `nextCursor` 中记录分页状态。
