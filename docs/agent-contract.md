...
MCP stdio 当前返回 `protocolVersion=2025-11-25`，`serverInfo.name=keepa_mcp`；客户端配置里的服务器别名仍可继续使用 `keepa`。公共 tool/prompt 名不再使用 `keepa.` 前缀；外部客户端迁移只应把调用名改成当前无前缀新名，不提供旧名 alias 或兼容映射。不要把客户端配置键从 `keepa` 强制改成 `keepa_mcp`；只有当外部客户端硬编码断言 `serverInfo.name == "keepa"` 时，才把断言更新为 `keepa_mcp`。`tools/list` / `resources/list` / `resources/templates/list` / `prompts/list` 支持 MCP cursor 分页：请求可传 `cursor`，Keepa-cli 额外支持 `limit` 便于本地调试；响应在 `_meta.total_count`、`_meta.has_more` 与 `nextCursor` 中记录分页状态。cursor 是不透明字符串，payload 内含 collection、offset、filter fingerprint 与 schema version；客户端不得解析或跨 `toolset/profile/allow_tools/exclude_tools` 等过滤条件复用。`toolset=all` 未显式传 `limit` 时默认只返回 8 个 starter tools，并通过 `nextCursor` 继续分页发现全集，避免无分页 all 响应过大。

## 4.1 MCP Streamable HTTP

`--mcp-http` 用于浏览器型或远程型 MCP client，本地默认监听 `127.0.0.1:8765`：

```powershell
.\.venv\Scripts\python.exe -m keepa_cli --mcp-http --mcp-http-host 127.0.0.1 --mcp-http-port 8765
```

HTTP endpoint 为 `POST /mcp`。客户端先发送 `initialize`，服务端返回 `MCP-Session-Id`；后续 POST 和 DELETE 必须携带该 header。adapter 支持浏览器 Origin allowlist、CORS preflight、`MCP-Protocol-Version=2025-11-25`、有界 `Keepa-MCP-Timeout-Ms`、显式不兼容 `Accept` 的 406 映射、显式非 JSON `Content-Type` 的 415 映射、空闲 session 1 小时清理、活动 HTTP session 上限 128、JSON-RPC notification/response 的 202 映射、application JSON-RPC error 的 200 映射，以及 `DELETE /mcp` 终止 session。当前未实现 server-sent event stream，因此 `GET /mcp` 明确返回 405。该入口只处理协议边界，所有有效 JSON-RPC 请求仍走 `handle_mcp_message -> AgentSession -> service`，不复制 tool/resource/prompt/Keepa 业务逻辑。

MCP Inspector 协议回归样例固化在 `tests/agent_eval_fixtures/mcp_inspector_protocol_fixture.json`，覆盖 `initialize`、分页 `tools/list`、`resources/list`、`prompts/list`、`resources/templates/list`、`tools/call` 参数错误和 `ping`。官方 Python MCP SDK adapter 对照见 `docs/architecture/mcp-python-sdk-adapter-comparison.md`；stdio 入口仍是本地桌面 Agent 默认入口，HTTP 入口是已落地的协议 adapter。隔离 SDK adapter 位于 `keepa_cli/agent/mcp_sdk_adapter.py`，可用 `scripts/compare_mcp_sdk_adapter_fixture.py` 对比当前 `--mcp` 与 adapter 兼容 handler 输出等价性；`tests/mcp_fixtures/mcp_sdk_adapter_filter_parity.json` 额外覆盖 `toolset/profile/allow_tools/exclude_tools/limit/cursor` parity。`scripts/check_mcp_performance_gate.py` 以 30 次 p95 记录 initialize、list、policy 与 fixture `products_get` 的 latency、JSON bytes、text fallback bytes、structuredContent bytes 和 cache hit latency，并支持 `--out` 把完整 JSON 写入 CI artifact；`scripts/summarize_mcp_performance_history.py` 从多轮 artifact 汇总真实历史 p95 / bytes max，输出可回写到 `THRESHOLDS` 的收紧建议。`scripts/check_mcp_output_schema.py` 只在门禁中校验代表性 outputSchema，不进入热路径。`scripts/smoke_mcp_sdk_adapter_client.py` 可通过官方 `ClientSession` 连接 `python -m keepa_cli.agent.mcp_sdk_adapter --stdio`；`scripts/check_mcp_sdk_adapter_typed_fixture.py` 会把 Inspector fixture step 映射成 SDK typed client 调用；`scripts/export_mcp_inspector_snapshot.py --check` 会导出并校验可复现 typed Inspector 快照；`scripts/check_mcp_quality_gate.py --require-sdk` 聚合 Agent eval、outputSchema、performance、adapter parity、SDK smoke、typed fixture 与 snapshot，CI 通过 `--performance-out artifacts/mcp-performance/...` 上传性能历史。由于官方 typed API 的 `list_*` 只支持标准 cursor，不支持 Keepa 扩展 `toolset/limit`，SDK adapter 默认压缩 tools、resources、resource templates、prompts 首页，分别以 `context_policy`、`keepa://context/policy`、`keepa://toolsets/{toolset}` 与 `product_research` 起手；完整 all-toolset 和完整资源/提示词清单通过 `nextCursor` 继续分页发现。SDK adapter 只有补齐生产入口的过滤与分页 parity 并持续通过该质量门禁后，才允许评估提升为生产入口。

MCP tool 只接受结构化 JSON 参数，不接受 CLI 字符串。工具 schema 包含 `title`、`inputSchema`、`outputSchema`、`execution.taskSupport=forbidden` 与 `annotations`；其中 `openWorldHint` 标出可能访问真实 Keepa / tracking API 的工具，`readOnlyHint=false` / `destructiveHint=true` 标出本地文件或 fixture/evidence 写入类工具。`reports_build` 与 `figures_research` 已在 `x-keepa.long_running_candidate` 标记为未来 MCP Tasks/progress 候选，当前仍不声明 Tasks 能力，普通 `tools/call` 只作为 fixture 或小输出路径，生产级长任务后续必须改走 Tasks/progress。返回同时包含 `structuredContent` 和 `content[0].text` JSON fallback；当 compact fallback 中包含 `mcp_resource_manifest` 时，还会追加官方 `resource_link` content block，便于支持该 block 的客户端直接读取重输出资源：
...
产品 live read 不进入默认 CI；手动验证入口是 `scripts/manual_live_product_read.py --asin <ASIN> --json`，缺省只做 dry-run 并输出 token budget 与 dry-run cache provenance。只有同时设置 `KEEPA_API_KEY` 并传入 `--yes-live` 时，脚本才执行单 ASIN `products.get`，并输出 `tokens.status` 前后摘要、SQLite cache provenance、cache hit 状态和脱敏结果摘要，不打印 API key 或完整商品原始响应。
...
`reports_build` 与 `figures_research` 的 tool metadata 会返回 `x-keepa.normal_tools_call_policy=fixture_or_small_output_only` 与 `x-keepa.future_task_support.target=required`。在当前 stdio 生产入口中它们仍通过普通 `tools/call` 执行离线小样本，后续 Streamable HTTP 或远程生产入口必须先实现 MCP Tasks/progress、请求级 timeout 与可恢复结果资源，再允许大文件报告或全量图表生成进入长任务路径。future task contract 已固定 `tasks/cancel`、`notifications/progress`、`tasks/result` 与 `keepa://tasks/{task_id}/result`，避免只靠 timeout 兜底。
