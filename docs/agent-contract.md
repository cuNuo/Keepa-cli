...
`reports_build` 与 `figures_research` 的 tool metadata 会返回 `x-keepa.normal_tools_call_policy=fixture_or_small_output_only` 与 `x-keepa.future_task_support.target=required`。在当前 stdio 生产入口中它们仍通过普通 `tools/call` 执行离线小样本，后续 Streamable HTTP 或远程生产入口必须先实现 MCP Tasks/progress、请求级 timeout 与可恢复结果资源，再允许大文件报告或全量图表生成进入长任务路径。
