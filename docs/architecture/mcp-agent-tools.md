...
- 官方 Python MCP SDK adapter 对照见 `docs/architecture/mcp-python-sdk-adapter-comparison.md`；结论是先保留当前 stdio 生产入口，再用 `keepa_cli/agent/mcp_sdk_adapter.py`、`scripts/compare_mcp_sdk_adapter_fixture.py` 和 `scripts/smoke_mcp_sdk_adapter_client.py` 对比 Inspector 与官方 SDK client 兼容性。
