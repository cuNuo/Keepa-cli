...
- 面向 agent：优先通过 `--json`、`--stdio` 和显式子命令提供稳定、无 ANSI、可解析、可组合的机器接口。
- 面向人类：默认打开易用的交互界面，能搜索、选择、预览 token 成本、查看详情、导出数据；但界面必须复用同一套 Agent-safe command service。
- 面向工程：可测试、可缓存、可审计、可在 GitHub Actions 中验证，且不泄露 Keepa API key。

核心原则：先形成 Agent 可调用的低成本、只读、可验证闭环，再扩展人类界面、高 token 成本和长期副作用能力。
...
### 2.3 必须补齐：Agent 适配硬门禁

后续要用这个 CLI 做 Agent，因此每个能力都必须满足以下门禁：

- 稳定 schema：成功、错误、预算、缓存、文件输出都有固定字段。
- 稳定 exit code：空结果成功，认证/参数/网络/API/解析失败非零。
- 有界输出：大结果默认写文件或分页，stdout 不无限刷屏。
- 可重放：`--dry-run` 输出足够重建请求的 redacted spec。
- 可组合：列表命令返回 ID、ASIN、sellerId、categoryId 等稳定后续输入。
- 可恢复：429 返回 `refill_in_ms`，高成本确认返回 `confirmation_required`。
- 可离线：fixture/offline 模式无需 Keepa key 也能跑测试和示例。
- 可审计：输出包含 `tokens_consumed`、`estimated_tokens`、`cache_hit`、`data_timestamp`。
- 凭据安全：任何 stdout/stderr/log/cache 都不得出现明文 API key。
- 双入口一致：`keepa-cli` 与 `kc` 对同一命令返回同一 schema。
...
建议第一版用 Rich/Textual 风格的终端界面，不急着做完整 Web UI。TUI 的成本更低，也更适合 CLI，但 TUI 必须调用同一套 Agent-safe command service。
...
- 把它当普通 CLI 做，后续再补 Agent 适配，会导致命令输出、错误处理和确认流程返工。
...
- 先冻结 Agent schema，再做 TUI；TUI 只能调用 command service，不直接调用 Keepa API。
...
目标：先防误操作，再扩展 API 能力，并让 agent 能在执行前判断成本。
...
- `--no-cache`
- `kc cache stats`
- `keepa-cli cache stats`
- `kc cache clear --dry-run`
- `keepa-cli cache clear --dry-run`

验收：

- 超预算请求拒绝执行。
- 缓存命中不访问网络。
- agent 输出包含 `estimated_tokens`、`worst_case_tokens`、`cache_hit`、`confirmation_required`。
...
### Phase 5：Agent `--stdio` 协议与 schema 冻结

目标：让 agent 能建立长会话，不需要反复启动进程，并把 MVP 级 schema 固定下来。
...
- schema 文档：success envelope、error envelope、event envelope、file output envelope。
...
- 协议测试不依赖真实 Keepa key。
- `keepa-cli --stdio` 与 `kc --stdio` 的事件 schema 完全一致。
...
- 2026-05-09 已落地 `finder query`、`deals query`、`sellers get`、`bestsellers get`、`topsellers list` 的 Agent-safe service / CLI / stdio / TUI 信息流。
- 当前只做 dry-run、fixture 与 `--out` 离线验证，不接真实 Keepa API。
- `finder.query`、`bestsellers.get`、`sellers.get` 已纳入 Agent schema snapshot。
...
4. `--json` / `--stdio` / Agent schema。
5. token 预算器。
6. product/category 查询。

做完再做：

1. TUI shell。
2. history export。
3. finder/deals/seller/rankings。
...
推荐路线：先完成 Agent-first MVP，再做 TUI，最后扩展高成本 API。

原因：

- MVP 可以最快验证 Keepa API、认证、预算、缓存、双入口和 Agent 调用协议。
- TUI 需要稳定的底层 service，否则容易变成重复逻辑。
- `--stdio` 必须和稳定 envelope、command service 同步完成，否则后续 Agent 会依赖不稳定契约。
