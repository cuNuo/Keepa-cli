---
name: keepa-cli
description: Use the Keepa CLI safely from Codex for live product research, fixture/dry-run testing, finder, deals, seller, graph, tracking, batch, report, cache, and cost-audit workflows.
---
...
Use fixture or `--dry-run` for development, regression tests, and budget previews. For real research, use live reads with `profile=live_read_allowed`; live Keepa calls require `KEEPA_API_KEY` or a local config token:
...
MCP `tools/list` defaults to `toolset=research`. Use `toolset=audit` for `keepa.audit_cost` and cassette tools, `toolset=reports` for local report/browse/SVG figure/brief export tools, `toolset=tracking-readonly` for read-only tracking, and `toolset=all` only when debugging schema discovery. Add `profile=live_read_allowed` for real research. Use `profile=offline_fixture_only` or `dry_run_default` for testing and budget previews: product/category/Finder/Deals/ranking tools remain discoverable, but `tools/call` must pass `fixture` or `dry_run`, otherwise it returns `profile_requires_fixture_or_dry_run`. Tools outside a profile still return `inactive_tool`. Research tools include `keepa.categories_finder_selection`, `keepa.research_graph_merge`, and `keepa.research_brief_export`.

MCP `resources/list` exposes `keepa://context/policy`, `keepa://schema/products-agent-view`, `keepa://schema/risk-taxonomy`, `keepa://schema/workflow-runtime-contract`, `keepa://fixtures/manifest`, `keepa://guides/cassette-promotion`, `keepa://guides/categories`, `keepa://guides/marketplaces`, `keepa://guides/agent-profile`, `keepa://evidence/recent`, and `keepa://workflow/runtime-contract`. `resources/templates/list` exposes `keepa://schema/{name}`, `keepa://fixtures/{name}`, `keepa://workflow/{encoded_params}/policy`, `keepa://research/{cache_key}`, `keepa://research/{cache_key}/brief`, `keepa://research/{cache_key}/graph`, `keepa://research/{cache_key}/figures`, `keepa://research/{cache_key}/figures/{figure_set}`, `keepa://graphs/{root}`, `keepa://chunk/{encoded_path}`, and `keepa://output/{encoded_path}`. Use `keepa://schema/risk-taxonomy` when an Agent needs to validate risk codes, severity, and evidence paths without loading the full product schema; use `keepa://guides/categories` and `keepa://guides/marketplaces` before asking users for category/marketplace IDs; use `keepa://guides/agent-profile` or `keepa.agent_profile_generate` to generate neutral MCP client config snippets and profile/toolset recommendations; use `keepa://workflow/runtime-contract` to discover resolver-enabled tools and follow its `schema_resource_uri` for validation, `keepa://workflow/{encoded_params}/policy` to read a compact `workflow_policy` from base64url JSON workflow params, `keepa://research/{cache_key}` to audit same-session cached results, `keepa://research/{cache_key}/brief` to reload an exported brief, `keepa://research/{cache_key}/figures` to generate all report-ready SVG resources from cached data, `keepa://research/{cache_key}/figures/history|compare|audit` to load only the figure group a report step needs, and `keepa://graphs/{root}` to audit graph sources before writing conclusions. If a tool text fallback includes `mcp_resource_manifest`, load `keepa://chunk/...` or `keepa://output/...` with `resources/read` instead of asking for the whole raw body again.

For general research Agents, read `keepa://context/policy`, call `keepa.resolve_research_target`, then call `keepa.query_research_context` before running live-capable product/category tools. `tools/list` accepts `allow_tools`, `exclude_tools`, and `profile` filters for small per-workflow schemas; use `profile=live_read_allowed` for real research and `profile=offline_fixture_only` only when fixture/dry-run execution is mandatory.

`workflow.plan` returns `workflow_inputs`, `artifacts`, `resource_templates`, and `workflow_policy` for MCP execution control. It supports `category-research`, `product-research`, `report-research`, `tracking-audit`, `inventory-audit`, `velocity-research`, and `market-opportunity`. Read it before running steps: apply `tool_discovery.params` to `tools/list`, follow `profile_switch_points`, treat `inactive_tools` as deliberate stage gates, connect steps with `input_refs` / `artifact_refs`, and only add `yes=true` after explicit confirmation for the listed `confirmation_policy.step_ids`. MCP `tools/call` accepts `resource_uri`, `resource_uris`, `artifact`, `artifacts`, `workflow_inputs`, and `workflow_context` to resolve prior outputs into concrete params; it also understands `artifact.output.path` / `artifact.data.output.path` and nested `workflow_context.steps` / `outputs` / `results` for local graph -> brief -> reports chains. Inspect `data.workflow_resolution` or `error.kind=missing_inputs`. `report-research` is local-only through the `reports` toolset; `tracking-audit` is read-only through `tracking-readonly`; business workflows are local-only through `business` + `offline_fixture_only`.

Business aliases hide low-level command details: use `keepa.find_fast_movers`, `keepa.inventory_audit`, or `keepa.market_opportunity` against an existing product JSON/fixture/resource/artifact. They return a conclusion-first `brief`, summary counts, and per-product `metrics` for velocity, seller competition, inventory risk, and cashflow proxy. Every estimate carries `method`, `version`, `inputs`, `confidence`, and `evidence_path`.
...
kc --json figures research --input batch.json --out-dir keepa-figures-history --figure-set history
```

`browse.snapshot` can render rows from raw product bodies or `research_graph` product nodes. `figures research` emits SVG resources plus source JSON with product comparison, real price/rank history lines when present, temporal window heatmaps, multi-ASIN normalized small multiples, risk codes, and graph entity summaries. Use `--figure-set all|history|compare|audit` to keep Agent reports focused; `all` keeps the backward-compatible overview, while scoped sets skip the overview and return only the requested single-chart SVG resources. Through MCP, `keepa.figures_research`, `keepa://research/{cache_key}/figures`, and `keepa://research/{cache_key}/figures/{figure_set}` expose SVG as `image/svg+xml` `keepa://output/...` resources suitable for reports. `reports build` embeds generated SVG automatically for markdown/json unless `--no-figures` is set, and also accepts `--figure-set`.
...
Export the final Agent handoff with `research brief` / `keepa.research_brief_export` after graph merge or multi-payload research. It returns `decision_summary`, `risk_summary`, `entity_graph_summary`, `follow_up_plan`, `evidence_links`, `external_signal_stub`, `ip_risk_inputs`, `claim_risk_inputs`, and `recommended_read_order` without rereading raw payloads. Keepa does not directly search external ads or patents; merge web/Exa/manual FTO evidence into those slots. For report workflows, pass `out_dir` when you want a commit-ready summary directory; runtime/raw files belong under runtime evidence and should stay uncommitted.
