...
Use `resources/list` before loading long docs. Stable resources are `keepa://context/policy`, `keepa://schema/products-agent-view`, `keepa://schema/risk-taxonomy`, `keepa://schema/workflow-runtime-contract`, `keepa://fixtures/manifest`, `keepa://guides/cassette-promotion`, `keepa://evidence/recent`, and `keepa://workflow/runtime-contract`. Use `resources/templates/list` to discover `keepa://schema/{name}`, `keepa://fixtures/{name}`, `keepa://workflow/{encoded_params}/policy`, `keepa://research/{cache_key}`, `keepa://research/{cache_key}/brief`, `keepa://research/{cache_key}/graph`, `keepa://research/{cache_key}/figures`, `keepa://graphs/{root}`, `keepa://chunk/{encoded_path}`, and `keepa://output/{encoded_path}`. Use `keepa://schema/risk-taxonomy` to validate risk codes, severity, and evidence paths, `keepa://workflow/runtime-contract` to discover resolver-enabled tools and follow its `schema_resource_uri` for validation, `keepa://workflow/{encoded_params}/policy` to read compact workflow execution policy from base64url JSON workflow params, `keepa://research/{cache_key}` to audit same-session cached results, `keepa://research/{cache_key}/brief` to reload an exported brief, `keepa://research/{cache_key}/figures` to generate SVG figure resources from cached research, and `keepa://graphs/{root}` to audit graph sources before writing conclusions. For tool results with `mcp_resource_manifest`, load `keepa://chunk/...` or `keepa://output/...` only when the summary is insufficient.
...
Use `reports.build` on the merged graph JSON when a human-readable or downstream relationship report is needed. Markdown includes an embedded SVG figure plus entity and relationship tables; JSON includes `figures` and `research_graph_report`. Pass `no_figures=true` / `--no-figures` only when the downstream renderer cannot handle images.

Use `figures.research` / `keepa.figures_research` when a report needs a visual artifact. It creates one SVG plus source JSON from product comparison rows, real price/rank history points when present, temporal window heatmaps, multi-ASIN normalized small multiples, risk codes, and graph entity counts. In MCP, read the `image/svg+xml` `keepa://output/...` resource from `mcp_resource_manifest` or from `keepa://research/{cache_key}/figures` and insert that SVG into the downstream report; load source JSON only for audit.
