...
Use `figures.research` / `keepa.figures_research` when a report needs visual artifacts. It creates SVG resources plus source JSON from product comparison rows, real price/rank history points when present, temporal window heatmaps, multi-ASIN normalized small multiples, risk codes, and graph entity counts. Use `figure_set=all` for the compatibility overview or `history|compare|audit` for scoped Agent sections. In MCP, read the `image/svg+xml` `keepa://output/...` resource from `mcp_resource_manifest`, `keepa://research/{cache_key}/figures`, or `keepa://research/{cache_key}/figures/{figure_set}` and insert that SVG into the downstream report; load source JSON only for audit.
