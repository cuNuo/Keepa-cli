...
隔离 SDK adapter spike 位于 `keepa_cli/agent/mcp_sdk_adapter.py`；运行 `python scripts/compare_mcp_sdk_adapter_fixture.py` 可对比它与当前 `--mcp` stdio handler 的输出等价性。可选 `mcp-sdk` extra 只用于 SDK 实验，不会替换生产 stdio 入口。
