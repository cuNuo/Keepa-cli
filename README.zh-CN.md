...
首批 MCP tools 为 `keepa.products_get`、`keepa.categories_search`、`keepa.categories_products`、`keepa.finder_query` 和 `keepa.audit_cost`。MCP 使用结构化 JSON 参数，不解析 CLI 字符串；返回包含 `structuredContent`、JSON text fallback、`cache_key`、`cache_hit` 与 `budget_ledger`，便于 Agent 在长流程中去重和审计 token 暴露。
