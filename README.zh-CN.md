...
隔离 SDK adapter 位于 `keepa_cli/agent/mcp_sdk_adapter.py`；运行 `python scripts/compare_mcp_sdk_adapter_fixture.py` 可对比兼容 handler 与当前 `--mcp` stdio 输出等价性，运行 `python scripts/smoke_mcp_sdk_adapter_client.py` 可用官方 Python MCP SDK `ClientSession` 真正连接验证。可选 `mcp-sdk` extra 启用 SDK 实验和 `python -m keepa_cli.agent.mcp_sdk_adapter --stdio` 入口，但不会替换生产 stdio 入口。
