...
`workflow plan` 现在会同时输出 `workflow_policy`，包含推荐 MCP `toolset`、会话 `profile`、allowed/inactive tools、profile switch points、确认策略、缓存建议和预算账本种子。Agent 应先按 `workflow_policy.tool_discovery` 收窄 `tools/list`，再根据 `profile_switch_points` 逐步切换 profile；高成本步骤只在用户确认后追加 `yes=true`。
