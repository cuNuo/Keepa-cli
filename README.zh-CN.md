...
Keepa CLI 将 Keepa API 工作流封装为稳定、可审计、适合 Agent 和人类共同使用的命令行界面。dry-run 和 fixture 用于开发测试、回归验证与预算预估；真实商品调研应使用 live read，显式配置 Keepa token，并遵守高成本端点的确认规则。
...
HTTP endpoint 是 `POST /mcp`。`initialize` 会返回 `MCP-Session-Id`；后续 POST 与 DELETE 必须携带该 header。adapter 会校验浏览器 Origin allowlist，支持 CORS 预检，接受 `MCP-Protocol-Version=2025-11-25`，按本地策略限制 `Keepa-MCP-Timeout-Ms`，显式不兼容的 `Accept` 返回 406，显式非 JSON 的 `Content-Type` 返回 415，空闲 session 1 小时后清理，活动 HTTP session 上限为 128，并支持 `DELETE /mcp` 终止 session。`GET /mcp` 当前返回 405，因为还未实现 server-sent event stream。HTTP adapter 只是协议层：JSON-RPC body 仍走 `MCPProtocolCore -> AgentSession -> service`，因此工具名、schema、cache、resources、prompts 与 token 等待提示都与 stdio 保持一致。

MCP 默认只返回紧凑的 `research` toolset，使用结构化 JSON 参数，不解析 CLI 字符串。`initialize` 返回 `serverInfo.name=keepa_mcp`；MCP 客户端配置别名仍可叫 `keepa`，但公共 tool/prompt 名不再带 `keepa.` 前缀。外部客户端迁移时只把调用名改成新名，例如 `context_policy`、`research_graph_merge`、`research_brief_export`，旧 `keepa.*` 名不会作为 alias 保留。`tools/list` 可传 `toolset=research/docs/audit/reports/tracking-readonly/all` 控制上下文大小，`allow_tools` 与 `exclude_tools` 可进一步收窄单次 schema；`toolset=all` 未显式传 `limit` 时只返回以 `context_policy` 起手的 8 个 starter tools 和 `nextCursor`，MCP cursor 是不透明字符串，不能跨不同过滤条件复用。`profile` 控制执行姿态：`live_read_allowed` 用于真实调研；`offline_fixture_only` 与 `dry_run_default` 会让产品、类目、Finder、Deals、榜单等研究工具保持可发现，但 `tools/call` 必须传 `fixture` 或 `dry_run`。安全 profile 下未传这些参数时返回 `profile_requires_fixture_or_dry_run`；真正不允许的工具仍返回 `inactive_tool`。research 覆盖产品、类目、本地 Finder scaffold、Finder、Deals、Seller、榜单、workflow plan、docs index/read、`research_graph_merge` 与 `research_brief_export`；audit 覆盖 cost、cassette sanitize/promote 与 `cassettes_promote_and_verify`；reports 覆盖图谱合并、本地报告、浏览快照、SVG 图表生成与 brief export；tracking 覆盖只读 tracking 与 cost audit。`products_compare` 默认返回 decision-summary `structuredContent`，完整同会话结果通过 `keepa://research/{cache_key}` 读取；CLI JSON 仍保留完整输出。Agent 结果会尽量提供统一 `research_graph`，工具 envelope 包含 `structuredContent`、JSON text fallback、`cache_key`、`cache_hit` 与 `budget_ledger`；重输出 fallback 还会追加 MCP `resource_link` content block。Inspector 风格协议回归规格位于 `tests/agent_eval_fixtures/mcp_inspector_protocol_fixture.json`；官方 Python MCP SDK adapter 对照位于 `docs/architecture/mcp-python-sdk-adapter-comparison.md`。
...
合并结果会给出 `source_weight`、重复节点、孤立节点、label/type 冲突诊断、`diff` 摘要和可选 `--prefer-source` 冲突解析，便于 Agent 在多来源结果不一致时先审计再写报告。`reports build` 可以直接读取 merged graph JSON，并在 Markdown 中自动嵌入 SVG 图、生成实体关系报告，或在 JSON 输出中写入 `figures` 与 `research_graph_report`。SVG 覆盖真实 price/rank 历史折线、窗口变化热图、多 ASIN 标准化 small multiples、风险枚举和图谱实体摘要；MCP 中可通过 `keepa://research/{cache_key}/figures` 从同一 session cache 生成图表资源。

`workflow plan` 现在会同时输出 `workflow_inputs`、`artifacts`、`resource_templates` 与 `workflow_policy`。`workflow_policy` 包含推荐 MCP `toolset`、会话 `profile`、allowed/inactive tools、profile switch points、确认策略、缓存建议和预算账本种子；每个 step 还会暴露 `input_refs` 与 `artifact_refs`，避免 Agent 从 CLI 字符串里推断中间产物。MCP `tools/call` 可直接传 `resource_uri`、`resource_uris`、`artifact`、`artifacts`、`workflow_inputs` 或 `workflow_context`，并会扫描 `workflow_context.steps`、`outputs`、`results`、`step_outputs` 与 `previous_outputs`，再把同一 session 的 `keepa://research/{cache_key}`、`keepa://research/{cache_key}/graph`、output path、`artifact.output.path` 与 inline artifact 解析成实际参数；`keepa://graphs/{root}` 保持为查找图谱来源的审计资源。成功响应会附带 `workflow_resolution`，缺少依赖时返回结构化 `missing_inputs`。支持 `category-research`、`product-research`、`report-research` 与 `tracking-audit`；其中 `report-research` 只走本地 reports toolset，`tracking-audit` 只走 tracking-readonly toolset。Agent 应先按 `workflow_policy.tool_discovery` 收窄 `tools/list`，再根据 `profile_switch_points` 逐步切换 profile；高成本步骤只在用户确认后追加 `yes=true`。
